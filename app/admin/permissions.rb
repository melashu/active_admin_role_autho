ActiveAdmin.register ::ActiveAdmin::Permission, as: "User Permission" do
  actions :index
  ...
  controller.resource_class.states.each_key do |state|
    batch_action state do |ids|
      ...
      redirect_back fallback_location: admin_root_url, notice: t("views.user_permission.notice.state_changed", state: state)
    end
  end

  collection_action :reload, method: :post do
    ::ActiveAdmin::ManagedResource.reload
    redirect_back(fallback_location: admin_root_url, notice: t("views.user_permission.notice.reloaded"))
  end

  action_item :reload do
    link_to t("views.user_permission.action_item.reload"), reload_admin_user_permissions_path, method: :post
  end
